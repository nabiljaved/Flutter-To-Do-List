...
class _ToDoScreenState extends State<ToDoScreen> {
  ...
  _loadCategories() async{                                                      //method to get categories
    var _categoryService = CategoryService();
    var categories = await _categoryService.getCategories();
    categories.forEach((result){
          setState(() {
            _categories.add(DropdownMenuItem(child: Text(result['name']) , value: result['name'],));
          });
    });
  }

  DateTime _date = DateTime.now();
  _selectTodoDate(BuildContext context) async{                                                      //select to do method
