...
class _CategoriesScreenState extends State<CategoriesScreen> {
  ...
  @override
  void initState(){                             //when app run it first of all get all categories
    super.initState();
    getAllCategories();
  }
...
//        print(result['description']);
  ...
  _showFormInDialog(BuildContext context)
  {
    return showDialog(context: context, barrierDismissible: true, builder: (param){
          return AlertDialog(
            actions: <Widget>[
              ...
              FlatButton(
                ...
                onPressed: () async{
                  ...
                  var result = await _categoryService.saveCategory(_category);          //save a category
                  if(result > 0){
                    Navigator.pop(context);
                    getAllCategories();
            ...
            title:
            Center(child: Text('Category Form')),
        ...
        actions: <Widget>[
          ...
          FlatButton(
            onPressed: () async{
              ...
              var result = await _categoryService.updateCategory(_category);      //edit a category
              if(result > 0){
                Navigator.pop(context);
                getAllCategories();
  ...
  _editCategory(BuildContext context, categoryId) async {                            //************get category by id*********
    ...
    return Scaffold(                                              //***************SCAFFOLD************************
      ...
      appBar: AppBar(
        leading: RaisedButton(
          elevation: 0.0,
          color: Colors.red,
          child: Icon(Icons.arrow_back, color: Colors.white,), //when back arrow is pressed
          onPressed: ()
          {
            Navigator.of(context).push(MaterialPageRoute(builder: (context) => HomeScreen())); //go back to previous screen
          },
        ),
      title: Text("categories"),
      ),
      body: ListView.builder(itemCount : _categoryList.length ,itemBuilder: (context, index){     //****build listview builder**
        return Card(
            child: ListTile(
          ...
          IconButton(icon: Icon(Icons.delete), onPressed: (){
           ...
           _deleteFormInDialog(context, _categoryList[index].id);                                                                //add when clicked
      ...
      floatingActionButton: FloatingActionButton(onPressed: (){         // call the alert dialog here
